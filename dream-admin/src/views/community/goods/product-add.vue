<template>
  <div class="app-container">
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="商品信息" name="first"  type="border-card">
        <el-form ref="elForm" :model="formData" :rules="rules" size="medium" label-width="100px" :disabled="isDisabled">
          ...
          <el-row :gutter="24">
            <el-col :span="8">
              <el-form-item label="商家名称" label-width="130px" prop="merchantName">
                <el-select v-model="formData.merchantName" 
                  style="width: 100%;"
                  clearable 
          ...
          <el-row :gutter="24">
            <el-col :span="24" v-if="['ONLINE','CURRENCY' ].includes(formData.business_method)">
              ...
              <el-form-item label="Banner图" label-width="130px" prop="productRotagramList">
                <div style="display: flex;">
                  <ul class="el-upload-list el-upload-list--picture-card">
                    <vueDraggable v-model="bannerList" @sort="dragSortBanner">
                      <li v-for="(item, index) in bannerList" :key="item.sort"
                        ...
                        <span class="el-upload-list__item-actions">
                          ...
                          <span class="el-upload-list__item-delete" @click="handleRemovebenner(index)">
                  ...
                  <el-upload 
                   ...
                   >
                    ...
                    <div slot="tip" class="el-upload__tip">支持多选</div>
                  </el-upload>
                </div>
    ...
    <el-dialog :visible.sync="previewDialog" width="30%">
    ...
    created() {
        ...
        this.details = this.$route.query.details
          if (this.details) {
            ...
            this.findProductDetails(this.productId);
      ...
      mounted() {
        ...
        methods: {
          ...
          findProductDetails(id) {
            ...
            findProductDetails(id).then(response => {
              if (response.code == 200) {
                ...
                this.bannerList = response.data.productRotagramEntityList.map((item, index) => {
                  return {
                    url: this.imgHost + item.imgUrl
                  }
                });
          ...
          dragSort() {
           ...
           //baner 排序
          dragSortBanner() {
            this.formData.productRotagramList = this.bannerList.map((item, index) => {
              return {
                imgUrl: item.url.substring(item.url.indexOf('files/')),
                sort: index + 1
              }
              ...
              if (res.code == 200) {
                for (let index in res.rows) {
                  this.arr.push({
                    ...
                    business_method: res.rows[index].mallMerchantEntity.businessMethod
                  })
                }
                let obj = {};
                ...
                if(param.merchantName && this.pages==1){
                  this.formData.business_method=res.rows[0].mallMerchantEntity.businessMethod;
                }
              } else {
                this.$message({
                  message: res.message,
          ...
          getDeliveryList(merchantId) {
            ...
            getDeliveryList(tempPara).then(response => {
          ...
          getCate(merchantId) {
            ...
            productCateList(param).then(response => {
          ...
          submitForm() {
            ...
            if (this.formData.skuFlag === '1') {
              if(this.skuItem.productCode === '' || this.skuItem.unitPrice === '' ||
          ...
          toLastView() {
            this.$router.push({
              path: '/community/product/product-list',
            })
          },
          isActive(route) {
           ...
           */
           handlePreview(file) {
            ...
            this.previewUrl= file.response==undefined? file.url: this.imgHost +file.response.data.url;
          ...
          /**
           * 
           * @param {*} index 删除benner图
           */
           handleRemovebenner(index){
