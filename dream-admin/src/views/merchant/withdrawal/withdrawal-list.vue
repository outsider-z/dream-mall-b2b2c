  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" >
      <el-form-item label="申请人" prop="userName">
        <el-input v-model="queryParams.userName" placeholder="请输入会员卡名称" clearable  />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="cardList" border>
      ...
      <el-table-column label="状态"  width="200" align="center" >
        <template slot-scope="scope">
          <dict-tag   :style="scope.row.withdrawStatus == 'APPROVING' ? '' : (scope.row.withdrawStatus == 'APPROVED' ? 'color: #67C23A;' : 'color:#E6A23C')"  :options="dict.type.sys_review_type" :value="scope.row.withdrawStatus" />       
         </template>
      </el-table-column>
      <el-table-column label="操作" align="center" width="300" fixed="right" class-name="small-padding fixed-width">
          <template slot-scope="scope" v-if="scope.row.id !== 1">
            <el-button 
            :disabled="scope.row.withdrawStatus=='APPROVING'?false:true" type="text" size="small "  @click="handleApply(scope.row)"
            :style="scope.row.withdrawStatus=='APPROVING'?'color: #67C23A;':''"
              v-hasPermi="['system:info:remove']">审核</el-button>
            <el-button :style="scope.row.withdrawStatus=='APPROVING'?'color: #E6A23C;':''" :disabled="scope.row.withdrawStatus=='APPROVING'?false:true" type="text" size="small "  @click="handleReject(scope.row)"
