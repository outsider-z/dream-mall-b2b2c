<template>
  <div class="app-container">
      ...
      <el-table-column label="操作" align="center" fixed="right" width="300" class-name="small-padding fixed-width">
        <template slot-scope="scope" >
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
            >修改</el-button>
          <el-button size="mini" type="text" style="color:rgb(245, 108, 108)" icon="el-icon-delete" @click="handleDelete(scope.row)"
           >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 商品规格 -->
    <el-dialog title="商品规格" :visible.sync="dialogTableVisible" width="50%">
      <el-table :data="tableColumnList.tableBodyList" height="500px" :border="true" ref="multipleTable">
        ...
        <el-table-column prop="totalUnit" label="拼团库存" align="center" width="150">
          <template slot-scope="scope">
            <el-input v-model="scope.row.totalUnit" size="mini" clearable type="number" />
          </template>
        </el-table-column>
        <el-table-column prop="activePrice" label="拼团价格" align="center" width="150">
          <template slot-scope="scope">
        ...
        <el-table-column prop="productAttrImg" label="图片" align="center"  width="100">
          <template slot-scope="scope">
            <el-popover placement="right" width="100" trigger="hover">
    ...
    <el-dialog title="商品规格" :visible.sync="singleTableVisible" width="50%" >
      <el-table :data="singleProList" height="500px" :border="true" ref="singleTable">
        <el-table-column prop="productCode" label="编码" align="center">
          <template slot-scope="scope">
            {{scope.row.productCode}}
          </template>
    ...
    <el-dialog title="修改规格" :visible.sync="editDialog" width="50%" >
      <el-table v-loading="loading" :data="editProductList" height="500px" :border="true">
        ...
        <el-table-column label="成团人数" prop="groupNum" align="center">
          <template slot-scope="scope">
            <el-input v-model="scope.row.groupNum" size="mini" clearable type="number"
              @input="editGroupNum(scope.row)" />
          </template>
        </el-table-column>
        <el-table-column label="商品规格" prop="redPacketNum" align="center">
          <template slot-scope="scope">
...
<script>
  ...
  import {
    merchantAddGroupProduct,
    merchantQueryGroupProduct,
    merchantModifyGroupProduct,
    merchantRemoveGroupProduct,
    queryGroupProductDetail
  ...
  export default {
    ...
    methods: {
      spikeProList() {
        merchantQueryGroupProduct(this.spikeQuery).then(res => {
      ...
      submitSku() {
        ...
        if (this.status == 'add') {
          ...
          merchantAddGroupProduct(para).then(response => {
          ...
          merchantModifyGroupProduct(para).then(response => {
            if (response.code == 200) {
      ...
      handleUpdate(row) {
        ...
        queryGroupProductDetail(para).then(response => {
      ...
      handleDelete(row) {
        ...
        this.$modal.confirm('是否确认删除该商品？').then(function() {
          return merchantRemoveGroupProduct(para);
        }).then(() => {
          this.spikeProList();
