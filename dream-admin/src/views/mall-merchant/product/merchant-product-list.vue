<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="productList" @selection-change="handleSelectionChange" :border="true">
      ...
      <el-table-column label="审核状态" prop="verifyStatus" width="200" align="center">
        <template slot-scope="scope" >
          <div :class="scope.row.mallProductEntity.verifyStatus=='PENDING_REVIEW'?'redColor':''" >
             {{scope.row.mallProductEntity.verifyStatus == 'PENDING_REVIEW' ? '待上架' : (scope.row.mallProductEntity.verifyStatus == 'APPROVED' ? '审核通过' : (scope.row.mallProductEntity.verifyStatus == 'FAIL' ? '审核未通过' : '审核中'))}}
          </div>
        </template>
      </el-table-column>
      <el-table-column label="审核详情" prop="verifyMsg" align="center">
        <template slot-scope="scope">
          {{scope.row.mallProductEntity.verifyMsg}}
        </template>
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      handleExport() {
        ...
        this.download('product/mall/product/list', {
          ...this.queryParams
