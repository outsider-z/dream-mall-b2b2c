<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
      <el-form-item label="商家名称" prop="productName">
        <el-select v-model="merchantName" filterable remote reserve-keyword placeholder="请输入关键词" clearable
    ...
    <el-table v-loading="loading" :data="productList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="审核状态" prop="verifyStatus" width="200" align="center">
        <template slot-scope="scope">
          {{scope.row.mallProductEntity.verifyStatus == 'PENDING_REVIEW' ? '待审核' : (scope.row.mallProductEntity.verifyStatus == 'APPROVED' ? '审核通过' : (scope.row.mallProductEntity.verifyStatus == 'FAIL' ? '审核未通过' : '审核中'))}}
        </template>
      </el-table-column>
